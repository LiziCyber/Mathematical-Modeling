...
for i=1:a
    for j=1:b
        for k=[3 5]
            if P(k).jud(i,j,1)
                W(i,j,k)=Tn(k).mit(i,j,H(i,j,k).ch(1));
            else
                W(i,j,k)=NaN;
            end
        end
...
m5=min(min(W(:,:,5)));
[Tx,Ty]=find(W(:,:,5)<m5+20&W(:,:,5)>m5+10);
sumH=zeros(1,num(5));
for i=1:num(5)
    for j=1:size(Tx,1)
        if H(Tx(j),Ty(j),5).ch(1)==i
            sumH(i)=sumH(i)+ss(Tx(j),Ty(j),5);
        end
    end
end

fid=fopen('ansmens-2.txt','a');
for i=1:num(5)
...
m5=min(min(W(:,:,3)));
[Tx,Ty]=find(W(:,:,3)<m5+20&W(:,:,3)>m5+10);
sumH=zeros(1,num(3));
for i=1:num(3)
    for j=1:size(Tx,1)
        if H(Tx(j),Ty(j),3).ch(1)==i
            sumH(i)=sumH(i)+ss(Tx(j),Ty(j),3);
        end
    end
end

fid=fopen('ans12mens2.txt','a');
for i=1:num(3)
    fprintf(fid,'%d ',sumH(i));
end
fprintf(fid,'\r\n');
