...
class MHP:
    ...
    def plot_rates(self, horizon=-1):
        ...
        for i in range(self.dim):
            ...
            axarr[row].set_ylabel('$\lambda(t)_{%d}$' % (i+1), fontsize=14)
            r = []
    ...
    def plot_rates2(self, horizon=-1):

        if horizon < 0:
            horizon = np.amax(self.data[:,0])

        f, axarr = plt.subplots(self.dim*2+1,1, sharex='col', 
        ...
        for i in range(self.dim):
            ...
            # plot rate
            r.append([self.get_rate(ct, i) for ct in xs])
            axarr[row].plot(xs, r[i], 'k-')
            axarr[row].set_ylim([-0.01, np.amax(r)+(np.amax(r)/2.)])
            axarr[row].set_ylabel('$\lambda(t)_{%d}$' % (i+1), fontsize=14)
        ...
        rep = np.array(r[1])/np.array(r[0])
        axarr[4].plot(xs, rep, 'k-')
        gap = np.max(rep)-np.min(rep)
        axarr[4].set_ylim([np.min(rep)-gap*0.2, np.amax(rep)+gap*0.2])
        axarr[4].set_ylabel('reputation' , fontsize=14)
